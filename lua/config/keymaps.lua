...
map("n", "<leader>ur", "<Cmd>nohlsearch<Bar>diffupdate<Bar>normal! <C-L><CR>", { desc = "Redraw / Clear Search / Update Diff" })
...
map({ "n", "x" }, "<leader>cf", function() format.format({ force = true }) end, { desc = "Format Buffer" })
...
map("n", "<leader>gL", function() Snacks.picker.git_log() end, { desc = "Open Git Log (CWD)" })
map("n", "<leader>gb", function() Snacks.picker.git_log_line() end, { desc = "Show Git Blame for Line" })
map("n", "<leader>gf", function() Snacks.picker.git_log_file() end, { desc = "Open Git File History" })
map("n", "<leader>gl", function() Snacks.picker.git_log({ cwd = root.git() }) end, { desc = "Open Git Log (Root Dir)" })
map({ "n", "x" }, "<leader>gB", function() Snacks.gitbrowse() end, { desc = "Open Git Browse URL" })
map({ "n", "x" }, "<leader>gY", function() Snacks.gitbrowse({ open = function(url) vim.fn.setreg("+", url) end, notify = false }) end, { desc = "Copy Git Browse URL" })
map("n", "<leader>qq", "<cmd>qa<cr>", { desc = "Quit All Windows" })
map("n", "<leader>ui", vim.show_pos, { desc = "Inspect Cursor Position" })
map("n", "<leader>uI", function() vim.treesitter.inspect_tree(); vim.api.nvim_input("I") end, { desc = "Inspect Treesitter Tree" })
map("n", "<leader>fT", function() Snacks.terminal() end, { desc = "Open Terminal (CWD)" })
map("n", "<leader>ft", function() Snacks.terminal(nil, { cwd = root.get() }) end, { desc = "Open Terminal (Root Dir)" })
map({ "n", "t" }, "<c-/>", function() Snacks.terminal.focus(nil, { cwd = root.get() }) end, { desc = "Focus Terminal (Root Dir)" })
